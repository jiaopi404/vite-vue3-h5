<template>
  ...
  >
    <!-- <div
      ...
      >{{ item.name }}</div
    > -->
    <div :class="`${prefixCls}-notice-item-container`" class="flex-1">
      <div
        :class="`${prefixCls}-notice-item`"
        class="flex items-center justify-between mt-1 mb-1"
        v-for="item in noticeListRef"
        :key="item.notice_id"
      >
        <div
          :class="`${prefixCls}-notice-item_name`"
          class="flex-1 truncate cursor-pointer lx-color--primary"
          :title="item.notice.title"
          @click="previewNotice(item.notice)"
          >{{ item.notice.title }}</div
        >
        <div :class="`${prefixCls}-notice-item_time`" class="flex-none"
          >【{{ formatToDateFormatter(item.notice.updateDateTime) }}】</div
        >
      </div>
    </div>
    <Pagination
