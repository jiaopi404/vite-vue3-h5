<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreateRole"> 添加供应商 </a-button>
      </template>
      <template #name="{ text, record }">
        <Tooltip :title="text">
          <div class="truncate cursor-pointer lx-color--primary" @click="handleToReport(record)">{{
            text
          }}</div>
        </Tooltip>
      </template>
  ...
  const [registerTable, { reload }] = useTable({
    ...
    beforeFetch: (queryInfo: any) => {
      ...
      if (queryInfo.regdate && queryInfo.regdate.length > 0) {
        queryInfo.regdate[0] = queryInfo.regdate[0].slice(0, 10);
        queryInfo.regdate[1] = queryInfo.regdate[1].slice(0, 10);
        appendQueryList({
