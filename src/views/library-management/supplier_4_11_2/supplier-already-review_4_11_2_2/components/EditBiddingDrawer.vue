<template>
  ...
  >
    <Tabs default-active-key="1" v-model:activeKey="currentKey" @change="changeTab">
      ...
      <TabPane key="2" tab="资质信息" v-if="supplierUserId" />
    </Tabs>
    ...
    <div v-if="currentKey === '2' && supplierUserId">
...
<script lang="ts" setup>
  ...
  import { editSupplierFormSchema } from '../schemas';
  ...
  const supplierUserId = ref(0);
  const recId = ref(0);
  const approveStatus = ref(1);
  const identifier = ref('sys_user');
  const getTitle = computed<string>(() => {
    return isUpdateRef.value ? '编辑供应商' : '添加供应商';
  });
  ...
  const [registerForm, { getFieldsValue, validate, resetFields, setFieldsValue, updateSchema }] =
    useForm({
      schemas: editSupplierFormSchema(),
      labelWidth: 150,
    ...
    try {
      ...
      const expertform = {
        ...
        biddingCompany: {
          ...
          ifShortlistedSuppliers: formData.ifShortlistedSuppliers,
          termOfValidity: formData.termOfValidity
            ? formData.termOfValidity.substring(0, 10) + ' 23:59:59'
            : 'null',
          ...
          ifRepairer: formData.ifRepairer,
          ifSupplier: formData.ifSupplier,
        },
      };
      // 保存
