<template>
  ...
  >
    ...
    <BasicForm @register="registerForm" style="resize: none" />
  </BasicDrawer>
</template>
<script lang="ts">
  ...
  import { formSchema } from '../procurement.data';
  import { useMessage } from '/@/hooks/web/useMessage';
  import { useUserStore } from '/@/store/modules/user';
  import { debouncePromise } from '/@/utils/commonServe';
  import { ProcurementManagement } from './procurementManagementModel';
  // 接口调用
  import {
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
          ...
          try {
            ...
            if (unref(isUpdate)) {
              ...
              updateSchema({
                ...
                rules: [
                  { required: true, message: '请输入采购文件名称', trigger: 'change' },
                  {
                    validator: debouncePromise(async (_, value) => {
                      const _params: ProcurementManagement = {
                        content: value || '',
                      };
                      ...
                      if (!res) {
                        return Promise.reject('您输入的采购文件名称重复');
              ...
              updateSchema({
                ...
                rules: [
                  { required: true, message: '请输入采购文件内容', trigger: 'change' },
                  {
                    validator: debouncePromise(async (_, value) => {
            ...
            if (unref(isAdd)) {
              ...
              updateSchema({
                ...
                rules: [
                  { required: true, message: '请输入采购文件内容', trigger: 'blur' },
                  {
                    trigger: 'blur',
                    validator: debouncePromise(async (_, value) => {
              ...
              updateSchema({
                ...
                rules: [
                  { required: true, message: '请输入采购文件名称', trigger: 'blur' },
                  {
                    trigger: 'blur',
                    validator: debouncePromise(async (_, value) => {
                      const _params: ProcurementManagement = {
      ...
      const getTitle = computed(() => (!unref(isUpdate) ? '添加采购文件模板' : '编辑采购文件模板'));
      // 保存
