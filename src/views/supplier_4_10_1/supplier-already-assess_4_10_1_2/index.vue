...
<script lang="ts" setup>
  ...
  import EditRoleDrawerVue from './components/EditBiddingDrawer.vue';
  ...
  const { appendQueryList, getHqlQueryDto, resetHqlQueryDto, setPage, setSorts } = useHqlQueryDto({
    hqlPageAndSortSumDto: {
      queryList: [
        ...
        { param: 'bidSection.supplierEvaluation', type: 'equal', value: [1] },
  ...
  const [registerTable, { reload }] = useTable({
    ...
    beforeFetch: (queryInfo: any) => {
      ...
      if (queryInfo.orgName) {
        appendQueryList({
          param: 'bidSection.biddingCompany.user.orgName',
          type: 'like',
  ...
  const auditing = async (record) => {
    openDrawer(true, {
      ...
      biddingCompany: toRaw(record.biddingCompany),
    });
  };
