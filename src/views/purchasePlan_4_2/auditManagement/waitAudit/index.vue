          ...
          value: [
            orParamsFormatter(
              " ((FIND_IN_SET('" +
                ...
                userInfo.id +
                " and ProcessId=(select ProcessId from con_contract where id =waitDealtWith.processObjectId) AND AuditOpinion is not NULL and result != 3 )=0,1=1))) or (select count(id) from sys_user where IfDelete=0 and UseMark=1 and ApproveStatus=1 and DepId=REPLACE(waitDealtWith.ASSIGNEE_,'depHeader_','') and id=" +
                userInfo.id +
  ...
  const [registerTable, { reload }] = useTable({
    ...
    beforeFetch: (queryInfo: any) => {
      ...
      if (queryInfo.planUseTime) {
        ...
        appendQueryList({
          param: 'waitDealtWith.planUseTime',
          type: 'le',
