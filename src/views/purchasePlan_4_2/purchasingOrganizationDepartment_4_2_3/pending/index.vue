<template>
  <div>
      ...
      <template #action="{ record }">
        <TableAction
          :actions="[
            {
              label: '处理',
              onClick: handlePending.bind(null, record),
            },
          ]"
        />
      </template>
    ...
    <!-- 待处理页面 -->
    <AddPending @register="registerDrawer" @success="handleSuccess" />
  </div>
</template>
<script lang="ts">
  ...
  // 配置数据
  import { columns, searchFormSchema } from './pending.data';
  import { useHqlQueryDto } from '/@/hooks/web/useHqlQueryDto';
  import { getProjectPage, getDepartmentStringById } from '/@/api/purchase/plan-purchase';

  // 引入页面组件
  import AddPending from './components/AddPending.vue';
  ...
  export default defineComponent({
    ...
    setup() {
        ...
        useHqlQueryDto({
          hqlPageAndSortSumDto: {
            ...
            queryList: [
              ...
              { param: 'project.status.code', type: 'in', value: [3] },
            ],
      ...
      // 打开待处理页面
      function handlePending(record: Recordable) {
        openDrawer(true, {
          record,
        });
      }
