<template>
  ...
  >
    ...
    <upload
      ...
      :action="apiUrl + '/conContractList/uploadConContractList'"
      :before-upload="beforeUpload"
      @change="handleChange"
      style="display: block; text-align: right"
    >
      <!-- :show-upload-list="false" -->
      <a-button type="primary" preIcon="carbon:cloud-upload"> 导入合同清单 </a-button>
    </upload>
...
<script lang="ts">
  ...
  import { importConContractList } from '/@/api/contractManagement/waitImprove';
    ...
    setup(props, { emit }) {
      ...
      const handleChange = (info) => {
        ...
        if (info.file.status === 'error') {
          createMessage.error('错误：导入合同清单失败！');
          changeOkLoading(false);
        }
      };
      ...
      const getImportUserMsg = async (url: string) => {
        try {
          let resData: any = await importConContractList({
            contractId: props.contractId,
            bidSectionId: props.bidSectionId,
            filePath: url,
          });
