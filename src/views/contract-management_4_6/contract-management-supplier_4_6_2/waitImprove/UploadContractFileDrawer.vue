...
<script lang="ts" setup>
  ...
  // BLOCK: common state
  const contractId = ref<Nullable<number>>(null);
  ...
  const getFileListRef = async () => {
    ...
    fileListRef.value = fileListRef.value.filter((item) => {
      if (props.statusCode) {
        return item.fileType.statusId?.code === props.statusCode;
      } else {
        return item.fileType.statusId?.code === '1';
      }
    });
